...
static /*rpacket_t*/void unpack(struct connection *c)
{
	...
	for(;;)
	{
		if(c->unpack_size <= sizeof(uint32_t))
			break;//return 0;
		buffer_read(c->unpack_buf,c->unpack_pos,reinterpret_cast<char*>(&pk_len),sizeof(pk_len));
...
{
	struct connection *c = reinterpret_cast<struct connection*>(calloc(1,sizeof(*c)));
	c->socket.sock = s;
